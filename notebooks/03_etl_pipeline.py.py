...
def get_env_var(name: str) -> str:
    ...
    if not value:
        raise ValueError(f"Variable de entorno faltante: {name}")
    return value
...
class FleetLogixETL:
    ...
    def chunked_iterable(self, iterable, size=5000):
        """Divide listas grandes en partes más pequeñas (batches)."""
            ...
            for _, row in customers.iterrows():
                cursor.execute("""
                    ...
                    WHEN NOT MATCHED THEN
                        ...
                        VALUES (s.customer_name, 'Individual', s.city, CURRENT_DATE(), 0, 'Regular')
    ...
    def load_facts(self, df: pd.DataFrame):
        ...
        try:
            ...
            # Insertar en batch
            for batch in self.chunked_iterable(fact_data, size=5000):
                cursor.executemany("""
    ...
    def _calculate_daily_totals(self):
        ...
        """)
            # Insertar totales SOLO para este batch
            cursor.execute("""
                INSERT INTO daily_delivery_totals (
                    total_date, total_deliveries,
                    on_time_rate, avg_delay_minutes,
                    total_revenue, etl_batch_id
                )
                SELECT
                    d.full_date,
